...
if (isset($_GET['delete'])) {
    ...
    if ($row) {
        ...
        $stmt1 = $conn->prepare("DELETE FROM doctors WHERE doctor_id=?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($user_id == 0) {
        ...
        // insert doctor
        $stmt2 = $conn->prepare("
            INSERT INTO doctors (user_id, specialty, contact, qualifications, experience)
            VALUES (?, ?, ?, ?, ?)
        ");
        $stmt2->bind_param("issss", $new_user_id, $specialty, $contact, $qualifications, $experience);
        $stmt2->execute();
        ...
        $stmt2 = $conn->prepare("
            UPDATE doctors
            SET specialty=?, contact=?, qualifications=?, experience=?
            WHERE user_id=?
        ");
        $stmt2->bind_param("ssssi", $specialty, $contact, $qualifications, $experience, $user_id);
        $stmt2->execute();
    }
...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("
        SELECT u.user_id, u.full_name, u.username, u.email,
               d.specialty, d.contact, d.qualifications, d.experience
        FROM users u
...
$doctors = $conn->query("
    SELECT u.user_id, u.full_name, u.email,
           d.doctor_id, d.specialty, d.contact, d.experience
    FROM users u
    JOIN doctors d ON d.user_id = u.user_id
    WHERE u.role='doctor'
");
...
<title>Doctors Management — Zaman Medical</title>
...
<aside class="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <a href="admindoctors.php" class="sb-link active"><span class="sb-icon"><i class="fa-solid fa-user-doctor"></i></span> Doctors</a>
...
<div class="main">
  ...
  <div>
    <div class="page-title">Doctor Management</div>
    <div style="font-size:13px;color:var(--text-3);margin-top:4px;">
      <?= date('l, d F Y') ?>
    </div>
  </div>
  <div class="btn-action">
    <i class="fa-solid fa-user-doctor"></i> Total Doctors: <?= $doctor_count ?>
  </div>
</div>
...
<div class="float-cards-row">
  <div class="float-card">
    ...
    <div>
      <div class="fc-label">Total Doctors</div>
      <div class="fc-value"><?= $doctor_count ?></div>
      <div class="fc-sub">Active medical staff</div>
    </div>
  </div>
</div>
...
<div class="lower-grid">
  ...
  <div class="form-card">
    <div class="form-header">
      ...
      <h3><?= $edit ? "Edit Doctor" : "Add New Doctor" ?></h3>
    </div>
...
<div class="form-group">
    ...
    <input type="password" name="password" class="form-control" required>
          ...
          <div class="form-group">
            ...
            <input type="text" name="specialty" class="form-control"
              value="<?= $edit['specialty'] ?? '' ?>" required>
          </div>

          <div class="form-group">
            <label>Contact</label>
            <input type="text" name="contact" class="form-control"
              value="<?= $edit['contact'] ?? '' ?>" required>
          </div>

          <div class="form-group">
            <label>Experience (Years)</label>
            <input type="number" name="experience" class="form-control"
              value="<?= $edit['experience'] ?? '' ?>" required>
          </div>

          <div class="form-group">
            <label>Qualifications</label>
            <input type="text" name="qualifications" class="form-control"
              value="<?= $edit['qualifications'] ?? '' ?>" required>
          </div>
        </div>
        ...
        <?php if ($edit): ?>
          <a href="admindoctors.php" class="btn-cancel">
  ...
  <div class="table-card">
    <div class="table-header">
      <h3><i class="fa-solid fa-user-doctor"></i> Doctors List</h3>
      <div class="record-count"><?= $doctor_count ?> Records</div>
    </div>
        ...
        <tbody>
          <?php while ($d = $doctors->fetch_assoc()): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($d['experience'] ?? '0') ?> yrs</td>
              <td>
                <div class="action-buttons">
                  <a href="?edit=<?= $d['user_id'] ?>" class="btn-edit">
                    <i class="fa-solid fa-pen"></i> Edit
                  </a>
                  <a href="?delete=<?= $d['user_id'] ?>"
                     class="btn-delete"
    ...
    <?php else: ?>
      <div class="empty-state">
        ...
        <p>Add your first doctor using the form.</p>
      </div>
    <?php endif; ?>
  </div>
