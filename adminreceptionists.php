...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM receptionists WHERE user_id=$id");
    $conn->query("DELETE FROM users WHERE user_id=$id");

    header("Location: adminreceptionists.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Fallback: if username left blank, derive one from the email prefix
    if ($username === '') {
    ...
    if ($user_id == 0) {
        ...
        // password_hash is required — NOT NULL, no default.
        // Generate a temp password, hash it, and show the plain version once
        // so it can be handed to the receptionist (they should change it on first login).
        $temp_password = bin2hex(random_bytes(4)); // e.g. "a1b2c3d4"
        ...
        // create receptionist
        $stmt = $conn->prepare("
            INSERT INTO receptionists (user_id, contact, shift, experience)
            VALUES (?, ?, ?, ?)
        ");
        $stmt->bind_param("isss", $new_id, $contact, $shift, $experience);
        $stmt->execute();
        $stmt->close();

        // Stash the temp password in the session so it can be shown once after redirect
        $_SESSION['temp_password_notice'] = "Account created for {$username}. Temporary password: {$temp_password}";
        ...
        $stmt = $conn->prepare("
            ...
            SET contact = ?, shift = ?, experience = ?
            WHERE user_id = ?
        ");
        $stmt->bind_param("sssi", $contact, $shift, $experience, $user_id);
        $stmt->execute();
        $stmt->close();
    }
...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("
        SELECT u.user_id, u.full_name, u.email, u.username,
               r.contact, r.shift, r.experience
        FROM users u
        JOIN receptionists r ON r.user_id = u.user_id
        WHERE u.user_id = ?
        LIMIT 1
    ");
...
// Count receptionists for stats
$receptionist_count = $receptionists->num_rows;
?>
...
<title>Receptionists Management — Zaman Medical</title>
...
.receptionists-table tbody td {
  padding: 14px 20px;
  font-size: 13px;
  color: var(--text-2);
  border-bottom: 1px solid var(--border);
}
.receptionists-table tbody tr:hover {
  background: rgba(198,42,42,0.04);
}
.shift-badge {
...
.shift-morning { background: rgba(34,197,94,0.12); color: #16a34a; }
.shift-evening { background: rgba(245,158,11,0.12); color: #d97706; }
.shift-night { background: rgba(59,130,246,0.12); color: #3b82f6; }
.action-buttons {
  display: flex;
  gap: 8px;
}
...
<aside class="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <a href="adminreceptionists.php" class="sb-link active"><span class="sb-icon"><i class="fa-solid fa-headset"></i></span> Receptionists</a>
...
<div class="main">
  ...
  <main class="content">
    ...
    <?php if (!empty($_SESSION['temp_password_notice'])): ?>
      <div style="background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.3);color:#15803d;padding:14px 20px;border-radius:14px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:10px;">
        <i class="fa-solid fa-circle-check"></i>
        <?= htmlspecialchars($_SESSION['temp_password_notice']) ?> — please share this securely and ask them to change it after logging in.
      </div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:baseline;">
      <div>
        ...
        <div style="font-size:13px;color:var(--text-3);margin-top:4px;">
          <?= date('l, d F Y') ?> · Staff Management
        </div>
      </div>

      <div style="font-family:'Fraunces',serif;font-size:28px;font-weight:700;color:var(--text-3);">
        <?= date('h:i A') ?>
      </div>
    </div>
    ...
    <div class="float-cards-row">
      <div class="float-card">
        <div class="fc-icon-wrap"><i class="fa-solid fa-headset"></i></div>
        <div>
          <div class="fc-label">Total Receptionists</div>
          <div class="fc-value"><?= number_format($receptionist_count) ?></div>
          <div class="fc-sub">Active staff</div>
        </div>
      </div>
    </div>
    ...
    <div class="form-card">
      <div class="form-header">
        ...
        <h3><?= $edit ? "Edit Receptionist" : "Add New Receptionist" ?></h3>
      </div>
      ...
      <div class="form-body">
        <form method="POST">
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <select name="shift" class="form-control" required>
                ...
                <option value="night" <?= ($edit['shift'] ?? '')=='night'?'selected':'' ?>>Night</option>
              </select>
            </div>
          ...
          <?php if ($edit): ?>
            <a href="adminreceptionists.php" class="btn-cancel">
    ...
    <div class="table-card">
      ...
      <div class="table-header">
        <h3><i class="fa-solid fa-list"></i> All Receptionists</h3>
        <div class="record-count"><?= $receptionist_count ?> records</div>
      </div>
      ...
      <table class="receptionists-table">
        ...
        <?php while($row = $receptionists->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              <span class="shift-badge shift-<?= strtolower($row['shift'] ?? '') ?>">
                <?= htmlspecialchars(ucfirst($row['shift'] ?? '')) ?>
              </span>
            </td>
            ...
            <td>
              <div class="action-buttons">
                <a class="btn-edit" href="?edit=<?= $row['user_id'] ?>">
