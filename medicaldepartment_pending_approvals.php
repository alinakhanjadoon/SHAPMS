...
if (isset($_POST['approve_id'])) {
    ...
    header("Location: medicaldepartment_pending_approvals.php");
    exit();
}
...
<title>Pending Approvals — Zaman Medical</title>
...
/* ─── PENDING TABLE (glassmorphic, matching dashboard) ──────────────────── */
.table-wrap{
...
/* Row avatar (matching dashboard) */
.row-av-w{display:flex;align-items:center;gap:10px;}
.row-av{
    width:33px;height:33px;border-radius:50%;flex-shrink:0;
    display:flex;align-items:center;justify-content:center;
    font-size:12px;font-weight:700;font-family:var(--ff-d);
}
.rav-doc{background:rgba(42,159,214,.2);color:var(--sky2);border:1px solid rgba(42,159,214,.28);}
.rav-nur{background:rgba(14,198,198,.2);color:var(--teal);border:1px solid rgba(14,198,198,.28);}
...
.badge-pen{background:rgba(245,158,66,.12);color:var(--warn);border:1px solid rgba(245,158,66,.26);}
...
.btn-approve,.btn-reject{
    ...
    border-radius:var(--r-sm);padding:7px 14px;font-size:12px;font-weight:600;
    font-family:var(--ff-b);transition:all .18s;
}
.btn-approve{background:rgba(48,217,136,.13);color:var(--success);border:1px solid rgba(48,217,136,.26);}
.btn-approve:hover{background:rgba(48,217,136,.25);box-shadow:0 0 12px rgba(48,217,136,.25);transform:translateY(-1px);}
.btn-reject{background:rgba(240,92,120,.13);color:var(--danger);border:1px solid rgba(240,92,120,.26);}
.btn-reject:hover{background:rgba(240,92,120,.25);box-shadow:0 0 12px rgba(240,92,120,.25);transform:translateY(-1px);}
...
.empty-icon{
    width:58px;height:58px;border-radius:50%;background:rgba(48,217,136,.1);
    border:1px solid rgba(48,217,136,.24);display:flex;align-items:center;
    justify-content:center;font-size:24px;color:var(--success);margin:0 auto 13px;
}
...
<div class="sidebar">
    ...
    <div class="nav">
        ...
        <?php
        $pendingCount = $conn->query("SELECT COUNT(*) FROM users WHERE status='pending' AND department_id=1")->fetch_row()[0] ?? 0;
        ?>
        <a class="active" href="medicaldepartment_pending_approvals.php">
...
<div class="main">
    ...
    <div class="topbar">
        <div class="topbar-left">
            ...
            <small><?= date("l, d F Y") ?> &nbsp;·&nbsp; Review pending staff approvals</small>
        </div>
    ...
    <div class="table-wrap">
        <div class="table-head">
            <h3><i class="fa-solid fa-hourglass-half" style="margin-right:8px;"></i> Pending Approvals</h3>
            <span class="badge badge-pen">
                <span class="pulse" style="width:6px;height:6px;border-radius:50%;background:var(--warn);display:inline-block;margin-right:5px;animation:pulse 1.4s infinite;"></span>
        ...
        <table>
            ...
            <tbody>
                <?php while($row = $pendingResult->fetch_assoc()):
                    $rInit = strtoupper(substr($row['full_name'],0,1));
                    $isDoc = $row['role'] === 'doctor';
                ?>
                <tr>
                    <td>
                        <div class="row-av-w">
                            <div class="row-av <?= $isDoc ? 'rav-doc' : 'rav-nur' ?>"><?= $rInit ?></div>
                            <strong><?= htmlspecialchars($row['full_name']) ?></strong>
                        </div>
                    </td>
                    <td>
                        <span class="badge <?= $isDoc ? 'doctor' : 'nurse' ?>">
        ...
        <?php else: ?>
            <div class="empty">
                ...
                <h3>All Caught Up!</h3>
                <p>No pending requests — all approvals are processed.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
