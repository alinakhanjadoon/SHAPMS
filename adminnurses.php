...
if (isset($_GET['delete'])) {
    ...
    header("Location: adminnurses.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($user_id == 0) {
       ...
       $username = strtolower(explode('@', $email)[0]) . rand(100, 999);
        $default_password = password_hash('nurse123', PASSWORD_DEFAULT);

        $conn->query("
            INSERT INTO users (username,password_hash,full_name,email,role,status)
            VALUES ('$username','$default_password','$full_name','$email','nurse','active')
        ");
        $new_user_id = $conn->insert_id;

        $conn->query("
            INSERT INTO nurses (user_id,contact,shift,experience)
            VALUES ($new_user_id,'$contact','$shift','$experience')
        ");
...
if (isset($_GET['edit'])) {
    ...
    $res = $conn->query("
        SELECT u.user_id,u.full_name,u.email,
               n.contact,n.shift,n.experience
        FROM users u
...
// Count nurses for stats
$nurse_count = $nurses->num_rows;
?>
...
<title>Nurses Management — Zaman Medical</title>
...
.nurses-table tbody td {
  padding: 14px 20px;
  font-size: 13px;
  color: var(--text-2);
  border-bottom: 1px solid var(--border);
}
.nurses-table tbody tr:hover {
  background: rgba(198,42,42,0.04);
}
.shift-badge {
...
<aside class="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <a href="adminnurses.php" class="sb-link active"><span class="sb-icon"><i class="fa-solid fa-user-nurse"></i></span> Nurses</a>
...
<header class="topbar">
  ...
  <div class="topbar-right">
    ...
    <div class="tb-greeting">
      <?= $greeting ?>, 
      <strong><?= htmlspecialchars(explode(' ', ($admin_name ?? 'Admin'))[0]) ?>?></strong> 👋
    </div>
  ...
  <main class="content">
    ...
    <h1 class="page-title">Nurses Management</h1>
    ...
    <div class="float-cards-row">
      <div class="float-card">
        ...
        <div>
          <div class="fc-label">Total Nurses</div>
          <div class="fc-value"><?= $nurse_count ?></div>
          <div class="fc-sub">Active staff members</div>
        </div>
      </div>
    </div>
    ...
    <section class="form-card">
      <div class="form-header">
        <i class="fa-solid fa-user-nurse"></i>
        <h3><?= $edit ? "Edit Nurse" : "Add New Nurse" ?></h3>
      </div>
      ...
      <div class="form-body">
        <form method="POST">
          ...
          <button type="submit" class="btn-submit">
            ...
            <?= $edit ? "Update Nurse" : "Save Nurse" ?>
          </button>

          <?php if ($edit): ?>
            <a href="adminnurses.php" class="btn-cancel">
    ...
    <section class="table-card">
      <div class="table-header">
        <h3><i class="fa-solid fa-table"></i> Nurses List</h3>
        <div class="record-count"><?= $nurse_count ?> Records</div>
      </div>
            ...
            <tr>
              ...
              <td>
                <div class="action-buttons">
                  <a href="?edit=<?= $row['user_id'] ?>" class="btn-edit">
